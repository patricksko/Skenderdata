...
class SSHFileTransferApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SSHCopyMaster v0.2")

        # Set a modern color scheme
        self.root.configure(bg="#303952")

        # Set the window size
        self.root.geometry("600x500")

        # Create a header label
        self.header_label = tk.Label(root, text="SSHCopyMaster v0.2", font=("Helvetica", 20), bg="#303952", fg="white")
        self.header_label.pack(pady=10)

        # Create UI elements with modern styling
        self.file_listbox = tk.Listbox(root, selectmode=tk.MULTIPLE, bg="#f4f4f4", bd=0, highlightthickness=0)
        self.file_listbox.pack(fill=tk.BOTH, expand=True, padx=20, pady=10)

        self.add_button = tk.Button(root, text="Add Files/Folders", command=self.add_files, bg="#2ecc71", fg="white", bd=0, padx=10, pady=5)
        self.add_button.pack()

        self.clear_button = tk.Button(root, text="Clear All", command=self.clear_all, bg="#e74c3c", fg="white", bd=0, padx=10, pady=5)
        self.clear_button.pack()

        self.transfer_button = tk.Button(root, text="Transfer Files", command=self.transfer_files, bg="#3498db", fg="white", bd=0, padx=10, pady=5)
        self.transfer_button.pack()

        # Apply flat design and subtle animations
        for button in [self.add_button, self.clear_button, self.transfer_button]:
            button.bind("<Enter>", lambda e, btn=button: btn.config(bg="#1abc9c"))
            button.bind("<Leave>", lambda e, btn=button: btn.config(bg=button["bg"]))
        ...
        self.add_button.bind("<Enter>", lambda e: self.add_button.config(bg="#c0392b"))
        self.add_button.bind("<Leave>", lambda e: self.add_button.config(bg="#e74c3c"))
    ...
    def transfer_files(self):
        ...
        with paramiko.Transport((host_details['host'], host_details['port'])) as transport:
            ...
            remote_folder_path = os.path.join(host_details['remote_path'], folder_name)
            try:
                sftp.mkdir(remote_folder_path)  # Create the remote folder
            except:
            ...
            for item in all_items:
                remote_item_path = os.path.join(remote_folder_path, os.path.basename(item))

                if os.path.isfile(item):  # Upload individual files
                    ...
                    with tqdm(total=total_size, unit='B', unit_scale=True, unit_divisor=1024) as pbar:
                        ...
                        sftp.put(item, remote_item_path, callback=update_progress)
                        pbar.update(total_size - pbar.n)

                    pbar.close()
                elif os.path.isdir(item):  # Upload contents of directories recursively
                    self.upload_directory(sftp, item, remote_item_path)
        ...
        for item in os.listdir(local_path):
            ...
            if os.path.isfile(local_item_path):
                ...
                try:
                    ...
                    pass  # Folder already exists
                self.upload_directory(sftp, local_item_path, remote_item_path)
